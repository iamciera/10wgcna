...
#use Dan Koenig's data. This is works to produce a network with three clusters, although it needs
#to be re-done.  The merging conflict is still present, but works to make a graph.  If
#you fix the merging conflict you get over 4,000 genes, I have not tried to run this, as it will
#take a long time!
...
colnames(superSOM)
#Subset only the columns that specify clusters and genes
superSOMsub <- superSOM[,c(3,22,23)]
names(superSOMsub)
#rename for merge
colnames(superSOMsub)[2] <- "som.unit.classif"
colnames(superSOMsub)[3] <- "som.distances"
names(superSOMsub)

#bring together lists from 
SOMclusters <- rbind(superSOMsub, SOMsub)
colnames(SOMclusters)

#isolate only gene names
SOMclusters <- as.data.frame(SOMclusters[,1])
...
#There are hardly any genes left!!! Check into this.
...
#pdf("top.hub.test.032615.test.pdf", useDingbats=FALSE) 
plot(subnet, 
